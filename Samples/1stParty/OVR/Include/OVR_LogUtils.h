...
#define OVR_VERBOSE(...) ((void)LogWithTag(ANDROID_LOG_VERBOSE, OVR_LOG_TAG, __VA_ARGS__))
...
#define OVR_VERBOSE(...) LogWithFileTag(ANDROID_LOG_VERBOSE, __FILE__, __VA_ARGS__)
...
#define OVR_VERBOSE(...) XLOG(DBG, ovrLogConvertPrintfToString(__VA_ARGS__))
...
// OVR_LOG_BUILD_DEBUG is for logging that should only be compiled in for debug builds
// with OVR_BUILD_DEBUG, cleaner than putting #ifdefs around OVR_LOG calls.
// This is not defined in user or userdebug STU builds, only in local /dev BUCK builds.
#if defined(OVR_BUILD_DEBUG)
