...
#else // defined(OVR_OS_ANDROID)

// Support JNI_TMP_ENV when testing on non-Android platforms. Other tests may already define
// a test mock for JNI_TMP_ENV, so only define it here if one does not already exist
