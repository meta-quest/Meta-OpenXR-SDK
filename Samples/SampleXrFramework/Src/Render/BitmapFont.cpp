...
static void UpdateFormat(
    ...
    if (format.Weight != format.LastWeight && format.Weight != 0xffffffff) {
        ...
        vertexParms[1] =
            (uint8_t)(std::clamp<float>(
                          fontParms.ColorCenter + fontInfo.CenterOffset + w.ColorCenterOffset,
                          0.0f,
                          1.0f) *
                      255);
        vertexParms[0] =
            (uint8_t)(std::clamp<float>(
                          fontParms.AlphaCenter + fontInfo.CenterOffset + w.AlphaCenterOffset,
                          0.0f,
                          1.0f) *
                      255);
    }
...
///			width, height, numLines, AsLocal( font ).GetFontInfo().FontHeight );
    ...
    uint8_t vertexParms[4] = {
        (uint8_t)(std::clamp<float>(
                      fontParms.AlphaCenter + fontInfo.CenterOffset + weightOffset, 0.0f, 1.0f) *
                  255),
        (uint8_t)(std::clamp<float>(
                      fontParms.ColorCenter + fontInfo.CenterOffset + weightOffset, 0.0f, 1.0f) *
                  255),
        //(uint8_t)( 0 ),
        (uint8_t)(std::clamp<float>(distanceScale, 1.0f, 255.0f)),
        (uint8_t)(std::clamp<float>(edgeWidth / 16.0f, 0.0f, 1.0f) * 255.0f)};
